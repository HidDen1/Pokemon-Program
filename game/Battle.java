...
public class Battle{
    ...
    private int playerDamageCalc(int attack, Pokemon wild, Options option, Player user, Pokedex pokedex){
        ...
        if (user.party[currentID].att[att].isPhysical()){
            ...
            damage = ((2 * user.party[currentID].getLevel() + 10) / 250) * (user.party[currentID].getSpAtk().getNum() / wild.getSpDef().getNum()) * (user.party[currentID].att[att].getPower() + 2) * multiplier;
            d = (int) damage;
        }
    ...
    public void wildAttack(Player user, Pokemon wild, Options option, Pokedex pokedex){
        ...
        if (wild.att[att].isPhysical()){
            ...
            damage = ((2 * wild.getLevel() + 10) / 250) * (wild.getSpAtk().getNum() / user.party[0].getSpDef().getNum()) * (wild.att[att].getPower() + 2) * multiplier;
        }
        damage = Math.round(damage);
        System.out.println("It did " + damage + " damage");
    ...
    public void wildLose(Pokemon wild, Player user, Options option, Pokedex pokedex){
        ...
        user.gainEVs(currentID, wild.getAtk().getEVG(), wild.getDef().getEVG(), wild.getSpAtk().getEVG(), wild.getSpDef().getEVG(), wild.getSpdEVG(), wild.getHpEVG());
    ...
    public void optionPokemon(int s, Player user, Pokemon wild, Options option, Pokedex pokedex){
        ...
        switch (c){
            ...
            case 2:
                ...
                System.out.println("Special Defense: " + user.party[s].getSpDef().getNum());
